  ...
  <view class="timeline-editor">
    ...
    <view v-for="event in events" :key="event.id" class="event-list-item">
      <view class="event-summary" @click="toggleEdit(event.id)">
        ...
        <text v-if="event.images?.length" class="image-badge"
          >📷 {{ event.images.length }}</text
        >
      </view>
    ...
    >
      <view class="event-editor">
        ...
        <view class="form-group">
          ...
          <input
            v-model="editForm.year_display"
            placeholder="如 1965、约1940年、1930年代"
            class="form-input"
          />
        </view>
        <view class="form-group">
          ...
          <input
            v-model="editForm.year_sort"
            placeholder="自动计算，可手动修正"
            type="number"
            class="form-input"
          />
          <text class="form-hint"
            >若希望按特定年份排序，请手动填写；留空则自动识别。</text
          >
        </view>
        <view class="form-group">
          ...
          <view class="tag-select">
            <text
              ...
              @click="
                editForm.event_type_label =
                  editForm.event_type_label === tag ? '' : tag
              "
              >{{ tag }}</text
            >
          </view>
          <input
            v-model="editForm.event_type_label"
            placeholder="自由输入标签"
            class="form-input"
          />
        </view>
        ...
        <view v-if="editingEvent" class="form-group">
          ...
          <view v-if="uploading" class="image-uploading">
            <text>上传中...</text>
          </view>
          <text
            v-if="editImages.length < 9"
            class="btn-add-image"
            @click="addImages"
            >+ 添加图片</text
          >
          ...
          <text class="form-hint"
            >支持 JPG/PNG/WEBP，单张 ≤ 10MB，不压缩保留原图</text
          >
        </view>
        ...
        <view class="editor-actions">
          ...
          <button
            class="btn-save"
            :disabled="
              !editForm.year_display || !editForm.event_title || saving
            "
            @click="saveEvent"
          >
            {{ saving ? "保存中..." : "保存" }}
    ...
    >
      <view class="event-editor confirm-box">
        ...
        <text class="confirm-text"
          >确定要删除这个事件吗？关联的图片也会被移除。</text
        >
        <view class="editor-actions">
          <button class="btn-cancel" @click="showDelConfirm = false">
            取消
          </button>
...
function getIcon(label) {
  ...
  for (const [key, icon] of Object.entries(ICON_MAP)) {
    if (label.includes(key) && key.length > matchedKey.length) {
      matched = icon;
      matchedKey = key;
    }
  }
...
async function saveCaption(img) {
  ...
  captionTimer = setTimeout(async () => {
    try {
      await familyStore.updateImageCaption(
        img.id,
        img.caption || "",
        props.memberId,
      );
...
async function saveEvent() {
  ...
  try {
    const payload = {
      ...
      year_sort:
        editForm.value.year_sort !== ""
          ? parseInt(editForm.value.year_sort)
          : null,
