<template>
  <view class="family-graph">
    ...
    <view class="graph-container" ref="graphRef"></view>
  </view>
</template>
...
function renderChart() {
  ...
  const coloredNodes = (graphData.nodes || []).map(node => ({
    ...node,
    symbolSize: node.category === 'center' ? (isLargeFont ? 65 : 60) : (isLargeFont ? 45 : 40),
    itemStyle: {
  ...
  const option = {
    series: [{
      ...
      data: coloredNodes,
      links: coloredLinks,
      edgeSymbol: (value, params) => params.data?.showArrow ? ['none', 'arrow'] : ['none', 'none'],
      categories: [
...
function switchView(view) {
  ...
  const depth = view === 'global' ? (familyStore.allMembers.length > 300 ? 3 : 5) : 3
...
<style>
.family-graph { width: 100%; height: 100%; position: relative; animation: fadeIn 0.3s ease; }
.graph-container { width: 100%; height: 100%; }
.view-switcher {
  position: absolute; top: var(--spacing-sm); left: var(--spacing-sm); z-index: 10;
...
.switch-option.active { background: var(--primary); color: #fff; }
.switch-option.disabled { opacity: 0.5; }
</style>
