      ...
      <view class="modal-content">
        ...
        <template v-if="mode === 'wizard'">
          ...
          <view class="step-indicator">
            <text class="step-dot" :class="{ active: wizardStep >= 1, done: wizardStep > 1 }">1</text>
            <view class="step-line" :class="{ done: wizardStep > 1 }"></view>
            <text class="step-dot" :class="{ active: wizardStep >= 2, done: wizardStep > 2 }">2</text>
          ...
          <view v-if="wizardStep === 1" class="wizard-step">
            ...
            <view class="form-group">
              ...
              <input v-model="wizard.self.name" placeholder="请输入您的姓名" class="form-input" />
            </view>
            <view class="form-group">
              <text class="form-label">性别</text>
              <view class="radio-group">
                <text class="radio" :class="{ active: wizard.self.gender === 1 }" @click="wizard.self.gender = 1">👨 男</text>
                <text class="radio" :class="{ active: wizard.self.gender === 2 }" @click="wizard.self.gender = 2">👩 女</text>
              </view>
            </view>
            <view class="form-group">
              <text class="form-label">是否在世</text>
              <text class="toggle-switch" :class="{ on: wizard.self.is_alive }" @click="wizard.self.is_alive = !wizard.self.is_alive">
          ...
          <view v-if="wizardStep === 2" class="wizard-step">
            ...
            <view class="form-group">
              ...
              <input v-model="wizard.father.name" placeholder="请输入您父亲的姓名" class="form-input" />
            </view>
            <view class="form-group">
              <text class="form-label">是否在世</text>
              <text class="toggle-switch" :class="{ on: wizard.father.is_alive }" @click="wizard.father.is_alive = !wizard.father.is_alive">
                {{ wizard.father.is_alive ? '🟢 在世' : '⚫ 已故' }}
              </text>
            </view>
            <view class="form-group">
              <text class="form-label">母亲姓名（选填）</text>
              <input v-model="wizard.mother.name" placeholder="可选录入母亲" class="form-input" />
            </view>
          </view>
          ...
          <view v-if="wizardStep === 3" class="wizard-step">
            ...
            <view class="form-group">
              ...
              <input v-model="wizard.grandpa.name" placeholder="请输入您祖父的姓名" class="form-input" />
            </view>
            <view class="form-group">
              <text class="form-label">祖母姓名（选填）</text>
              <input v-model="wizard.grandma.name" placeholder="可选录入祖母" class="form-input" />
            </view>
            <view class="form-group">
              <text class="form-label">管理员联系方式（选填）</text>
              <input v-model="wizard.adminContact" placeholder="如：张明，微信：zhangming123" class="form-input" />
            </view>
          </view>

          <view class="wizard-actions">
            <button v-if="wizardStep > 1" class="btn-cancel" @click="wizardStep--">上一步</button>
        ...
        <template v-if="mode === 'findme'">
          ...
          <view class="search-box">
            ...
            <input v-model="searchQuery" placeholder="输入您的姓名..." class="search-input" @input="onSearchDebounced" />
          </view>
          <view v-if="searchResults.length > 0" class="search-results">
            ...
            >
              <view class="avatar-placeholder-sm">{{ member.name.charAt(0) }}</view>
              <view class="search-item-info">
                <text class="search-item-name">{{ member.name }}</text>
                <text class="search-item-parent">{{ getParentHint(member) }}</text>
              </view>
              <text v-if="selectedMemberId === member.id" class="check-mark">✅</text>
            </view>
          </view>
          ...
          <view class="modal-footer">
            ...
            <button class="btn-confirm" :disabled="!selectedMemberId" @click="confirmIdentity">✅ 确认这是我</button>
          </view>
        </template>
      </view>
    </view>
...
async function nextWizardStep() {
  ...
  try {
    ...
    if (wizard.value.adminContact) {
      await supabase.from('families').update({ admin_contact: wizard.value.adminContact }).eq('id', VITE_FAMILY_ID)
    }
    ...
    if (wizard.value.grandma.name) {
      ...
      // 同步祖父配偶
      await supabase.from('members').update({ spouse_id: grandma.id }).eq('id', grandpa.id)
    ...
    if (wizard.value.mother.name) {
      ...
      await supabase.from('members').update({ spouse_id: mother.id }).eq('id', father.id)
      familyStore.allMembers = familyStore.allMembers.map(m =>
    ...
    uni.showToast({ title: '建谱失败: ' + e.message, icon: 'error', duration: 3000 })
  }
}
...
function confirmIdentity() {
  ...
  const member = familyStore.allMembers.find(m => m.id === selectedMemberId.value)
  if (member) {
...
function skipOnboarding() {
  uni.showToast({ title: '您可以在右上角「👤」随时设置身份', icon: 'none', duration: 2000 })
  emit('skip')
}
</script>
...
.modal-title { font-size: 22px; font-weight: bold; text-align: center; }
.modal-subtitle { font-size: 14px; color: #6B6B6B; text-align: center; margin: 8px 0 20px; }

/* 向导 */
.step-indicator { display: flex; align-items: center; justify-content: center; gap: 0; margin-bottom: 24px; }
.step-dot {
  width: 32px; height: 32px; border-radius: 50%;
  background: #eee; color: #999; display: flex; align-items: center; justify-content: center;
  font-size: 14px; font-weight: bold;
}
.step-dot.active { background: #8B1A1A; color: #fff; }
.step-dot.done { background: #27AE60; color: #fff; }
.step-line { width: 40px; height: 2px; background: #eee; }
.step-line.done { background: #27AE60; }
.step-title { font-size: 16px; font-weight: bold; margin-bottom: 16px; }
.wizard-actions { display: flex; gap: 12px; margin-top: 24px; }
.btn-next { flex: 1; height: 48px; background: #8B1A1A; color: #fff; border-radius: 12px; }

/* 搜索 */
.search-box {
  display: flex; align-items: center;
  border: 1px solid #ddd; border-radius: 12px; padding: 0 12px; margin-bottom: 16px;
}
...
.search-item.selected { background: #FFF8F0; border: 1px solid #C9A96E; }
.search-item-name { font-size: 16px; font-weight: 500; }
.search-item-parent { font-size: 13px; color: #6B6B6B; }
.search-empty { text-align: center; color: #999; padding: 20px 0; }
.avatar-placeholder-sm {
  width: 40px; height: 40px; border-radius: 50%; background: #8B1A1A;
  color: #fff; display: flex; align-items: center; justify-content: center;
  font-size: 18px; font-weight: bold;
}
.modal-footer { display: flex; gap: 12px; }
.btn-skip { flex: 1; height: 48px; background: #fff; border: 1px solid #ddd; border-radius: 12px; }
.btn-confirm { flex: 1; height: 48px; background: #8B1A1A; color: #fff; border-radius: 12px; }
...
.form-input { width: 100%; height: 44px; border: 1px solid #ddd; border-radius: 12px; padding: 0 12px; }
.radio-group { display: flex; gap: 12px; }
.radio {
  padding: 8px 20px; border: 1px solid #ddd; border-radius: 12px; font-size: 14px;
  min-width: 48px; min-height: 48px; display: flex; align-items: center; justify-content: center;
}
.radio.active { border-color: #8B1A1A; background: #FFF5F5; color: #8B1A1A; }
.toggle-switch {
  display: inline-flex; align-items: center; padding: 8px 16px;
  border-radius: 12px; font-size: 14px; border: 1px solid #ddd;
  min-width: 48px; min-height: 48px;
}
.toggle-switch.on { border-color: #27AE60; background: #F0FFF4; }
</style>
