  ...
  <view class="page-index">
    ...
    <view v-else-if="familyStore.isEmpty" class="empty-state">
      <template v-if="userStore.isAdmin">
        ...
        <text class="empty-desc">目前家族尚未录入成员，点击下方按钮开始修谱。</text>
        <button class="btn-start" @click="showOnboarding = true">+ 添加先祖</button>
      </template>
      <template v-else>
        <text class="empty-icon">🏛️</text>
        <text class="empty-title">家族数据正在建立中</text>
        <text class="empty-desc">目前家族尚未录入成员，请联系管理员添加数据。</text>
        <text v-if="adminContact" class="contact-info" @click="copyContact">{{ adminContact }}</text>
    ...
    <view v-else class="main-content">
      <view class="top-nav">
        ...
        <view class="nav-actions">
          ...
          <text v-if="userStore.isAdmin" class="action-icon" @click="goToAdmin">⚙️</text>
...
async function loadAllMembers() {
  ...
  try {
    ...
    const { data: familyInfo } = await supabase.rpc('get_family_info', { family_id: VITE_FAMILY_ID })
...
function toggleFontSize() {
  ...
  familyStore.buildGraph(userStore.myMemberId, 3)
  uni.showToast({
    title: userStore.fontSizePreference === 20 ? '已切换为大字号' : '已恢复标准字号',
    icon: 'none',
    duration: 1000,
  })
}
...
async function logout() {
  ...
  uni.showToast({ title: '已退出，请重新输入访问码或登录', icon: 'none', duration: 1500 })
...
function copyContact() {
  if (adminContact.value) {
    uni.setClipboardData({
      ...
      success: () => uni.showToast({ title: '已复制管理员联系方式', icon: 'success', duration: 1500 }),
    })
  }
}
...
<style>
.page-index { min-height: 100vh; background: var(--bg-page); }
.top-nav {
  display: flex; justify-content: space-between; align-items: center;
  height: 56px; padding: 0 var(--spacing-lg); background: var(--primary); color: #fff;
  box-shadow: 0 2px 8px rgba(139,26,26,0.3);
}
.family-name { font-size: var(--font-size-lg); font-weight: bold; font-family: var(--font-family-title); letter-spacing: 2px; }
.nav-actions { display: flex; gap: var(--spacing-xs); }
.action-icon {
  font-size: var(--font-size-lg); min-width: 48px; min-height: 48px;
...
.action-icon.active { color: var(--accent-gold); font-weight: bold; border-bottom: 2px solid var(--accent-gold); }
.graph-area { width: 100%; height: calc(100vh - 56px); animation: fadeIn 0.3s ease; }

/* 空状态 */
.empty-state {
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  min-height: 80vh; padding: 40px var(--spacing-lg); animation: fadeInUp 0.4s ease;
}
.empty-icon { font-size: 64px; margin-bottom: var(--spacing-md); }
.empty-title { font-size: var(--font-size-xl); font-weight: bold; font-family: var(--font-family-title); margin-bottom: var(--spacing-sm); }
.empty-desc { font-size: var(--font-size-md); color: var(--text-secondary); text-align: center; margin-bottom: var(--spacing-lg); line-height: 1.6; }
.btn-start {
  height: 48px; padding: 0 var(--spacing-xl); background: var(--primary); color: #fff;
  border-radius: var(--radius-md); font-size: var(--font-size-md);
}
.contact-info {
  font-size: var(--font-size-base); color: var(--primary); text-decoration: underline;
  padding: var(--spacing-sm); min-width: 48px; min-height: 48px;
  display: flex; align-items: center; justify-content: center;
}
</style>
